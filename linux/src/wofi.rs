...
pub fn select_entry(entries: &[Entry], preselect: Option<usize>) -> io::Result<Option<usize>> {
    if entries.is_empty() {
        return Ok(None);
    }
    ...
    command.arg("--dmenu").arg("--prompt").arg("Clipboard");

    if let Some(index) = preselect {
        command.arg("--preselect").arg(index.to_string());
    }
